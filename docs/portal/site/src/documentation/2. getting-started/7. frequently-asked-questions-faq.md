...
You may encounter an `Insufficient storage` message when installing the APK on the emulator.
...
When you have found the `config.ini` file, add the following line:
...
Locate the `config.ini` file as explained in section
[Insufficient storage when installing the APK](#insufficient-storage-when-installing-the-apk).

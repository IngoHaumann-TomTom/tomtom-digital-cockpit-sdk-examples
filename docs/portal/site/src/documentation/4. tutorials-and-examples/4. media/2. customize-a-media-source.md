...
interface would display the [IviMediaItem](TTIVI_INDIGO_API)`.title` field of a playable media item
in a visually more prominent fashion (in bold and with a bigger font size) and the
...
specified in a [PolicyProvider](TTIVI_INDIGO_API), which will be used only when browsing
...
The new `HeartActionMediaControl` should be then specified in a
...
The name must be displayed fully as "Example Internet Radio", if possible, and only if not possible
...
This is an example defining how `exampleInternetRadioPolicyProvider` might look like with all the

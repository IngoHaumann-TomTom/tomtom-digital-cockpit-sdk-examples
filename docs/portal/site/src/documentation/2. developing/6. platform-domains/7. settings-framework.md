...
A settings service is a special case of IVI service, that has only properties. Properties can be
either __read-only__ or __writable__. See `IviSetting` annotation for details.
...
See also [Configuration framework](6.%20configuration-framework-domain.html).
...
see [IVI services](ivi-services.md).
The difference is that the interface contains only properties annotated with the `IviSetting`
...
inherits `IviSettingsServiceBase` class, and contains methods to manage settings, that may be
...
For such a setting, the implementation has to define only the configuration key and the setting key.
...
open class FooSettingsServiceBase {
    ...
    /**
     * Reads the value of [foo] property from the persistent storage.
     */
    ...
    /**
     * Writes the value of [foo] property to the persistent storage.
     */
    ...
    /**
     * Reads the value of [bar] property from the persistent storage.
     */
     // It is an abstract method because the type of the property is not supported by the settings
     // management API.
    ...
    /**
     * Writes the value of [bar] property to the persistent storage.
     */
     // It is an abstract method because the type of the property is not supported by the settings
     // management API.
    ...
    /**
     * Updates the value of [bar] property.
     */
...
It is worth to add a dependency to a settings service interface for a deployment configuration of
...
the same process.
See documentation for IVI service deployment for details.

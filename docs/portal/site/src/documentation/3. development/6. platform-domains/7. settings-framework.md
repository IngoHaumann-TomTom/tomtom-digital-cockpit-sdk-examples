...
[IviSettingsServiceBase](TTIVI_INDIGO_API) class. See also the [Configuration
framework](/indigo/documentation/development/platform-domains/configuration-framework).
...
implements an IVI service interface; see [IviService](TTIVI_INDIGO_API). The difference is that the
interface only contains properties annotated with the [IviSetting](TTIVI_INDIGO_API) annotation.
Clients may access the service using the service API instance, created with
`<interface>.createApi()`.

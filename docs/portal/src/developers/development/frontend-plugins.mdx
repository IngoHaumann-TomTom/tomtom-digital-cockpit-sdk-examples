...
[_off-the-shelf_](/tomtom-digital-cockpit/developers/development/introduction#off-the-shelf-components-or-stock-components),
...
[System UI](/tomtom-digital-cockpit/developers/development/system-ui) processes and displays when needed.
...
panel's `Panel.onAttached` method with a [`PanelAttachment`](TTIVI_PLATFORM_API) instance which
contains the [`NavigablePanelContext`](TTIVI_PLATFORM_API) data. When a task panel's view model
implements the [`NavigablePanelViewModel`](TTIVI_PLATFORM_API) interface, a fully populated
[`NavigationBarViewModel`](TTIVI_PLATFORM_API) is available in the task panel's view model in
[`NavigablePanelViewModel.getNavigationBarViewModel`](TTIVI_PLATFORM_API)` based on this data.
When your task panel's view model extends from [`FrontendViewModel`](TTIVI_PLATFORM_API),
...
[`NavigablePanelViewModel.getNavigationBarViewModel`](TTIVI_PLATFORM_API) to the
...
[Main process panel](/tomtom-digital-cockpit/designers/system-ui/main-process-panels). Or, if you
...
[`NotificationPanel.Priority`](TTIVI_PLATFORM_API). The system UI uses it to determine when and how
...
[System UI](/tomtom-digital-cockpit/developers/development/system-ui). Or, if you want to start building a
notification, see
...
panel's `Panel.onAttached` method with a [`PanelAttachment`](TTIVI_PLATFORM_API) instance which
contains the [`NavigablePanelContext`](TTIVI_PLATFORM_API) data. When a modal panel's view model
implements the [`NavigablePanelViewModel`](TTIVI_PLATFORM_API) interface, a fully populated
[`NavigationBarViewModel`](TTIVI_PLATFORM_API) is available in the modal panel's
[`NavigablePanelViewModel.getNavigationBarViewModel`](TTIVI_PLATFORM_API) based on this data. When
...
layout by assigning [`NavigablePanelViewModel.getNavigationBarViewModel`](TTIVI_PLATFORM_API) to
...
[Anatomy](/tomtom-digital-cockpit/designers/system-ui/main-process-panels#Anatomy) section in the
...
provides a helper class [`IviFragment.ViewFactory`](TTIVI_PLATFORM_API) that takes care of common

...
[`VehicleProperties.ListProperty`](TTIVI_PLATFORM_API)` (or any other `Property` type) is used. 
If the area _is_ relevant, then the [`VehicleZoneProperty`](TTIVI_PLATFORM_API) type is used, 

...
[discoverable IVI service](/tomtom-digital-cockpit/developers/development/ivi-services#discoverable-services)
...
[AlexaHandlerService.onMessageReceived](TTIVI_PLATFORM_API) method is called.
...
3. Decode the AASB message into an instance of the data class using the
   [`parseAasbMessage`](TTIVI_PLATFORM_API) function. For example:<br/>
...
2. Create an instance of this class, encode it as a JSON string and send it using the
   ...
   [createAasbReplyHeader](TTIVI_PLATFORM_API) helper function to create the AASB reply header. 
   For example:<br/>
...
[createAasbRequestNoPayload](TTIVI_PLATFORM_API) or [createAasbRequestHeader](TTIVI_PLATFORM_API) helper
functions. Typically, in order to send an AASB "Publish" message, you would:
...
2. Create an instance of this class, encode it as a JSON string and send it using the
   [AacsSenderWrapper](TTIVI_PLATFORM_API) helper class. You would normally also use the
   [createAasbRequestHeader](TTIVI_PLATFORM_API) helper function to create the AASB Publish header. 
   For example:
...
- TomTom Digital Cockpit [Customize the Alexa Car Control Feature](/tomtom-digital-cockpit/developers/tutorials-and-examples/voice-personal-assistant/customize-the-alexa-car-control-feature)

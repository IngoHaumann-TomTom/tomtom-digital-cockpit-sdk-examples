...
The language change is called via the function [`setSystemLocale`](TTIVI_PLATFORM_API)
...
[`setClockTimeFormat`](TTIVI_PLATFORM_API)`.`
by passing as parameters the `Context` instance and the desired setting in the following enum
format:
- [`TimeFormat.CLOCK_12H`](TTIVI_PLATFORM_API) for a 12 hour format.
- [`TimeFormat.CLOCK_24H`](TTIVI_PLATFORM_API) for a 24 hour format.

It is also possible to call the API with the enum value [`TimeFormat.UNSET`](TTIVI_PLATFORM_API).
...
[`getClockTimeFormat`](TTIVI_PLATFORM_API) and pass in the `Context`. The API will return a
[`TimeFormat`](TTIVI_PLATFORM_API) enum representing the current system setting stored in 
`[Settings.System.TIME_12_24]` as either `TimeFormat.CLOCK_24H`, `TimeFormat.CLOCK_12H` or 
`TimeFormat.UNSET` in the case that no setting has

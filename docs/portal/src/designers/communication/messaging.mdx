...
SMS messages can often be from unknown senders and contain advertisements or promotions that are
distracting the user while driving. Message notifications, therefore, are filtered, and the
following table lists the sender types that are filtered out and when the notifications are shown.
...
![messaging user flow](images/communication-messaging-user-flow.png)
